<script lang="ts">
	...
	const tagGroups = groupByTags(getAllPosts('blog'));
...
<svelte:head>
	...
	<meta name="description" content="Browse blog posts by tags" />
</svelte:head>

<main class="mx-auto max-w-4xl p-6">
	<h1 class="mb-8 text-3xl font-bold">Blog Tags</h1>
					...
					<div class="posts-list ml-5 mt-2">
						{#each group.posts as post, i}
							<article class="archive-item">
								...
								<a href="/blog/{post.slug}" class="ml-1 hover:underline">{post.title}</a>
							</article>
						{/each}
					</div>
				{/if}
